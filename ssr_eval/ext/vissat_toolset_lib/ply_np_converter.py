...
def np2ply(
    vertex, out_ply, color=None, comments=None, text=False, use_double=False
):
    if use_double:
    ...
    if text:
        ...
        if comments is None:
            ...
            PlyData(
                [el], text=True, text_fmt=text_fmt, comments=comments
            ).write(out_ply)
    else:
        if comments is None:
