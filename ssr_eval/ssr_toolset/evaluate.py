...
from ssr_eval.ext.vissat_toolset_visualization.plot_error_map import (
    plot_error_map,
)
from ssr_eval.ext.vissat_toolset_visualization.plot_height_map import (
    plot_height_map,
)
from ssr_eval.ext.vissat_toolset_visualization.plot_error_dist import (
    plot_err_dist,
)
from ssr_eval.ext.vissat_toolset_lib.dsm_util import read_dsm_tif
...
def calc_score(source, target):
    ...
    completeness = (
        np.sum(np.less(abs_err, completeness_threshold))
        / valid_target_cell_cnt
    )
...
def align_worker(
    return_dict, proc_idx, source, target, target_pad_width, xy_candi_shifts
):
    target = target[
        target_pad_width:-target_pad_width, target_pad_width:-target_pad_width
    ]
    h, w = target.shape[:2]
...
def align(
    ...
    num_processes = len(
        xy_candi_shifts
    )  # actual number of processes to be launched
    ...
    print(
        "before alignment, median err: {}, completeness: {}".format(
            median_err, completeness
        )
    )
    ...
    median_err, completeness = calc_score(source_shifted, target)
    print(
        "best dx, dy, dz: {}, {}, {}, best median err: {}, completeness: {}".format(
    ...
    with open(os.path.join(out_dir, "offset.txt"), "w") as fp:
        ...
        fp.write(f"median_err_rounded (meters): {median_err:.3f} \n")
        fp.write(f"median_err_rounded (meters): {100 * completeness:.1f} \n")
    ...
    plot_height_map(
        target,
        os.path.join(out_dir, "target_after_align.jpg"),
        save_cbar=True,
        force_range=(min_val, max_val),
    )
    plot_height_map(
        source_shifted,
        os.path.join(out_dir, "source_after_align.jpg"),
        save_cbar=True,
        force_range=(min_val, max_val),
        maskout=np.isnan(target),
    )
    plot_error_map(
        err, os.path.join(out_dir, "error_map.jpg"), force_range=(-2.0, 2.0)
    )
    return median_err, completeness
...
def evaluate(source_tif, target_tif, out_dir, max_processes):
    ...
    assert np.abs(
        source_meta["east_resolution"] - target_meta["east_resolution"]
    ) < 1e-5 and np.abs(
        source_meta["north_resolution"] - target_meta["north_resolution"]
        < 1e-5
    )

    cell_physical_size = (
        target_meta["east_resolution"],
        target_meta["north_resolution"],
    )
    ...
    target = np.pad(
        target,
        (
            (target_pad_width, target_pad_width),
            (target_pad_width, target_pad_width),
        ),
        mode="constant",
    ...
    ul_x = (
        target_meta["ul_easting"] - source_meta["ul_easting"]
    ) / source_meta["east_resolution"]
    ul_y = (
        source_meta["ul_northing"] - target_meta["ul_northing"]
    ) / source_meta["north_resolution"]
